...
class PrivateKeyEntityBuilderTest {
    ...
    void buildFromEmptyBundle_noKeys() {
        ...
        final List<Entity> entities = builder.build(new Bundle(), BundleType.ENVIRONMENT, INSTANCE.getDocumentBuilder().newDocument());
    ...
    void build() {
        ...
        PrivateKeyEntityBuilder builder = new PrivateKeyEntityBuilder(keystoreHelper);
        final List<Entity> entities = builder.build(bundle, BundleType.ENVIRONMENT, INSTANCE.getDocumentBuilder().newDocument());
        ...
        Assertions.assertEquals(KeyStoreType.GENERIC.generateKeyId("test"), entity.getId());
