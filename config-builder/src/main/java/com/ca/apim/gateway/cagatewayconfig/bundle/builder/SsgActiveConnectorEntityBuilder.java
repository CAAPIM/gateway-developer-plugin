        ...
        switch (bundleType) {
            case DEPLOYMENT:
                final Stream<Entity> activeConnectorOnlyMappings = entities.entrySet().stream().map(ssgActiveConnectorEntry ->
                        EntityBuilderHelper.getEntityWithOnlyMapping(EntityTypes.SSG_ACTIVE_CONNECTOR, bundle.applyUniqueName(ssgActiveConnectorEntry.getKey(), BundleType.ENVIRONMENT, false), generateId(ssgActiveConnectorEntry.getValue())));
            ...
            case ENVIRONMENT:
                final Stream<Entity> activeConnectors = entities.entrySet().stream().map(ssgActiveConnectorEntry ->
                        buildActiveConnectorEntity(bundle, bundle.applyUniqueName(ssgActiveConnectorEntry.getKey(), BundleType.ENVIRONMENT, false), ssgActiveConnectorEntry.getValue(), document));
