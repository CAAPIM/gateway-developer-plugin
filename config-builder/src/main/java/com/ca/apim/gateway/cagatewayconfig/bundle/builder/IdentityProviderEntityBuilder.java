        ...
        switch (bundleType) {
            ...
            case ENVIRONMENT:
                return entities.entrySet().stream().map(identityProviderEntry ->
                        buildIdentityProviderEntity(bundle, bundle.applyUniqueName(identityProviderEntry.getKey(),
                                bundleType, false), (IdentityProvider)identityProviderEntry.getValue(), document)
                ).collect(Collectors.toList());
            default:
