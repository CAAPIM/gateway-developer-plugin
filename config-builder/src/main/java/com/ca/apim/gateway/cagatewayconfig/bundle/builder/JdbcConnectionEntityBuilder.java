        ...
        switch (bundleType) {
            case DEPLOYMENT:
                return entities.entrySet().stream()
                        .map(e -> EntityBuilderHelper.getEntityWithOnlyMapping(EntityTypes.JDBC_CONNECTION,
            ...
            case ENVIRONMENT:
                return entities.entrySet().stream().map(e ->
                        buildEntity(bundle.applyUniqueName(e.getKey(), bundleType, false), (JdbcConnection)e.getValue(),
                                document)
                ).collect(Collectors.toList());
            default:
